...
3. 本地 preview、Docker 与原生浏览器 200% 缩放的已测交互、视口及资源检查见[前端验收记录](docs/design/frontend-acceptance.md)。该记录只覆盖列明的状态与视口；概念稿、组件接入和有限浏览器检查不能推定真实用户验证、全部缩放及对比度组合或 Jev 全部网络行为已通过。
